...
import com.tg.educonnect.appointmentservice.exceptionhandler.GlobalExceptionHandler;
...
public class AppointmentService {
    ...
    AppointmentService(GlobalExceptionHandler globalExceptionHandler) {
	...
	public Appointment updateAppointmentStatus(Long appointmentId, String status) {
		Appointment appointment=getAppointmentById(appointmentId);
		AppointmentStatus statusref=AppointmentStatus.valueOf(status.toUpperCase());
		if(!appointment.getStatus().equals(statusref)) {
			appointment.setStatus(statusref);
			return appointmentRepository.save(appointment);
		}
		throw new CustomException("unable to update status");
	}
	
	public List<Appointment> getAppointmentByCourseId(Long courseId) {
	    CourseDTO courseDTO = courseClient.getCourseById(courseId);
	    if (courseDTO == null || courseDTO.getCourseId() == null) {
	        throw new CustomException("Course service unavailable or course not found for ID: " + courseId);
	    }
	    List<Appointment> appointmentList = appointmentRepository.findByCourseId(courseDTO.getCourseId());
	    if (appointmentList.isEmpty()) {
	        throw new CustomException("No appointments found for course ID: " + courseId);
	    }
	    return appointmentList;
	}
